...
public class ProduitsResource {
	...
	public Response getMonumentById(@PathParam("id") Long id) {
		Produits produits = produitsDao.getProduitById(id);
		if (produits != null)
	...
	public Response updateMonument(@PathParam("id") Long id, Produits produitsAModifier) {
		produitsAModifier.setIdentifiant(id);
		produitsDao.updateProduit(produitsAModifier);
	...
	public void deleteMonumentById(@PathParam("id") Long id) {
		produitsDao.deleteProduitById(id);
	}
}
