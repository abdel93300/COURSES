...
@Table(name = "PRODUITS")
@NamedQueries({
		@NamedQuery(name = "Produits.findAll", query = " SELECT m FROM Monument m ORDER BY m.nom "),
		@NamedQuery(name = "Produits.findAllByListecoursesId", query = " SELECT m FROM Monument m JOIN m.ville v WHERE v.id = :id ORDER BY m.nom "),
		@NamedQuery(name = "Produits.deleteById", query = " DELETE FROM Monument m WHERE m.identifiant = :id") })
